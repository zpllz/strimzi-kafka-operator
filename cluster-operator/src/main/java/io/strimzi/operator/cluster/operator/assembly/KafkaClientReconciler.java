...
import io.strimzi.api.kafka.model.KafkaClientResources;
 ...
 */
public class KafkaClientReconciler {
    private static final ReconciliationLogger LOGGER = ReconciliationLogger.create(KafkaClientReconciler.class.getName());
    ...
    private boolean existingKafkaClientCertsChanged = false;
    ...
    public KafkaClientReconciler(
        ...
        this.kafkaClient = KafkaClient.fromCrd(reconciliation, kafkaAssembly, versions, supplier.sharedEnvironmentProvider);
    ...
    private Future<Void> serviceAccount() {
        return serviceAccountOperator
                .reconcile(
                        ...
                        KafkaClientResources.deploymentName(reconciliation.name()),
                        kafkaClient != null ? kafkaClient.generateServiceAccount() : null
                ).map((Void) null);
    }
    ...
    private Future<Void> certificatesSecret(Clock clock) {
        if (kafkaClient != null) {
            return secretOperator.getAsync(reconciliation.namespace(), KafkaClientResources.secretName(reconciliation.name()))
                    .compose(oldSecret -> {
                        return secretOperator
                                .reconcile(reconciliation, reconciliation.namespace(), KafkaClientResources.secretName(reconciliation.name()),
                                        kafkaClient.generateSecret(clusterCa, Util.isMaintenanceTimeWindowsSatisfied(reconciliation, maintenanceWindows, clock.instant())))
                                .compose(patchResult -> {
                                    if (patchResult instanceof ReconcileResult.Patched) {
                                        ...
                                        existingKafkaClientCertsChanged = ModelUtils.doExistingCertificatesDiffer(oldSecret, patchResult.resource());
                                    } else {
                                        existingKafkaClientCertsChanged = false;
                                    }
            ...
            return secretOperator
                    .reconcile(reconciliation, reconciliation.namespace(), KafkaClientResources.secretName(reconciliation.name()), null)
                    .map((Void) null);
        }
    }
        ...
        if (isNetworkPolicyGeneration) {
            return networkPolicyOperator
                    .reconcile(
                            ...
                            reconciliation.namespace(),
                            KafkaClientResources.deploymentName(reconciliation.name()),
                            kafkaClient != null ? kafkaClient.generateNetworkPolicy() : null
                    ).map((Void) null);
        } else {
            return Future.succeededFuture();
        }
    ...
    private Future<Void> deployment(boolean isOpenShift, ImagePullPolicy imagePullPolicy, List<LocalObjectReference> imagePullSecrets) {
        if (kafkaClient != null) {
            Deployment deployment = kafkaClient.generateDeployment(isOpenShift, imagePullPolicy, imagePullSecrets);
            ...
            return deploymentOperator
                    .reconcile(reconciliation, reconciliation.namespace(), KafkaClientResources.deploymentName(reconciliation.name()), deployment)
                    .compose(patchResult -> {
                        if (patchResult instanceof ReconcileResult.Noop)   {
                            ...
                            if (existingKafkaClientCertsChanged || clusterCa.certsRemoved()) {
            ...
            return deploymentOperator
                    .reconcile(reconciliation, reconciliation.namespace(), KafkaClientResources.deploymentName(reconciliation.name()), null)
                    .map((Void) null);
        }
    }
    ...
    private Future<Void> kafkaExporterRollingUpdate() {
        return deploymentOperator.rollingUpdate(reconciliation, reconciliation.namespace(), KafkaClientResources.deploymentName(reconciliation.name()), operationTimeoutMs);
    }
    ...
    private Future<Void> waitForDeploymentReadiness() {
        if (kafkaClient != null) {
            return deploymentOperator.waitForObserved(reconciliation, reconciliation.namespace(), KafkaClientResources.deploymentName(reconciliation.name()), 1_000, operationTimeoutMs)
                    .compose(i -> deploymentOperator.readiness(reconciliation, reconciliation.namespace(), KafkaClientResources.deploymentName(reconciliation.name()), 1_000, operationTimeoutMs));
        } else {
            return Future.succeededFuture();
        }
    }
